...
def upload_new_blob(full_name, path, blob_b64, message, branch="main"):
    url = f"https://api.github.com/repos/{full_name}/contents/{path}"
    payload = {
        "message": message,
        "content": blob_b64,
...
def main():
    ...
    print("ppb branch:", ppb_branch, "file sha:", ppb_file_sha, "ktc file sha:", ktc_file_sha)

    print("\n=== upload coinbase tutorial (new file) ===")
    r = upload_new_blob("mknight2690-sys/Polly-Poly-Bot", "docs/hermes-coinbase-setup.html",
                        tutorial_b64, "Add Hermes on Coinbase beginner tutorial (Windows & Mac)",
                        branch=ppb_branch or "main")
    print(json.dumps(r, indent=2, ensure_ascii=False))

    print("\n=== upload sales page (replace file) ===")
