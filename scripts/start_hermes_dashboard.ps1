...
$cmd = "@`"$pythonw`" `"$root\scripts\launch_hermes_gui.py`"`""
if(-not (Test-Path $desktop)){ $desktop = [Environment]::GetFolderPath('Desktop') }
$lnk = Join-Path $desktop 'Hermes Trader.lnk'
...
$shortcut.Save()
$lnk2 = Join-Path $desktop 'Hermes Dashboard.lnk'
...
$shortcut2.Save()
$lnk3 = Join-Path $desktop 'Stop Hermes.lnk'
$shortcut3 = (New-Object -ComObject WScript.Shell).CreateShortcut($lnk3)
$shortcut3.TargetPath = 'powershell'
$shortcut3.Arguments = "-NoProfile -Command `"& '$root\Scripts\stop_hermes_gui.ps1'`""
