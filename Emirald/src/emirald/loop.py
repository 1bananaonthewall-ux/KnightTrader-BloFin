...
from .nous_client import LLMUnavailable, NousClient, NousClientPool
...
class EmiraldLoop:
    def __init__(self, secrets: Secrets, config: AppConfig):
        ...
        nous_keys = getattr(secrets, "nous_portal_keys", None) or [secrets.nous_portal_key]
        if len(nous_keys) <= 1:
            self.llm = NousClient(
                api_key=nous_keys[0],
            ...
            logger.info("Using NousClientPool with %d keys", len(nous_keys))
            self.llm = NousClientPool(
                keys=nous_keys,
                model=config.llm.model,
                base_url=config.llm.base_url,
                timeout_seconds=config.llm.timeout_seconds,
    ...
    def startup(self) -> None:
        ...
        logger.info("starting Emirald in %s mode", mode)
    ...
    def _run_one_tick_inner(self, tick: int, t0: float) -> None:
        ...
        if raw_reason is None:
            ...
            try:
                ...
                logger.warning("LLM unavailable, using fallback decisions: %s", e)
            except Exception:  # noqa: BLE001
        ...
        if raw_reason is not None and not raw_reason.startswith("llm_unavailable"):
            eq = None
            try:
        ...
        if raw is None:
            raw_reason = raw_reason or "llm_unavailable"
            try:
                fb = demo_fallback_decisions(snapshot, self.paper)
                if fb.decisions:
                    logger.info(
                        "%s fallback producing %d decisions after LLM failure",
                ...
                logger.exception("fallback crashed after LLM failure")
                decision = CycleDecision.empty("fallback_crashed")
        else:
            # 4. Parse + validate
            decision = parse(raw, snapshot)
            decision = risk.apply(decision, snapshot)  # currently a no-op

            # If the LLM produced no actionable JSON or zero-size decisions, use playbook momentum fallback
            ...
            actionable = [d for d in decision.decisions if float(getattr(d, "sz", 0) or 0) > 0]
            if not actionable:
                open_n = (
        ...
        actionable = [
            ...
            if (getattr(d, "action", "") or "").lower() not in {"hold", "no_trade", "wait"}
            and float(getattr(d, "sz", 0) or 0) > 0
        ]
        if actionable:
    ...
    def _refresh_summary(self, snapshot) -> None:
        ...
        prompt = (
            "You are Emirald. Below are your older trades (the ones you no "
    ...
    def run_forever(self) -> None:
        ...
        def _shutdown(signum, frame):
            ...
            try:
                ...
                console.print("[green]Emirald sleeping.[/green]")
            sys.exit(0)
        ...
        mode = "DEMO" if self.demo else "LIVE"
        console.print(
            f"[bold green]Emirald is awake ({mode}). Interval: {interval}s (+/- {jitter}s).[/bold green]"
        )
        scheduler.start()
