"""Emirald dashboard — live Blofin account (demo/paper supported).

Streaming design:
- 500ms UI poll hits lightweight cached endpoints.
- Live account/positions are cached ~1s so UI slides without hammering Blofin.
- Journal/log endpoints are append-aware to avoid full re-renders.
"""
...
app = FastAPI(title="Emirald Dashboard")
...
DEFAULT_LOG = BASE_DIR / "data" / "emirald.log"
PID_FILE = BASE_DIR / ".emirald.pid"
...
_JOURNAL_CACHE: dict[str, Any] = {"rows": [], "ts": 0.0}
_JOURNAL_CACHE_TTL = 0.5

_LOG_CACHE: dict[str, Any] = {"text": "", "ts": 0.0}
_LOG_CACHE_TTL = 0.5
...
def _live_account_overlay() -> dict[str, Any]:
    ...
    try:
        ...
        from emirald.market_data import fetch_account, fetch_positions
        ...
        client = BlofinClient(
            ...
            position_mode=getattr(_cfg, "position_mode", "net") or "net",
        )
        ...
        pos_rows = [
            {
                ...
                "liquidation_px": getattr(p, "liquidation_px", None),
            }
            for p in positions
        ]
        _LIVE_CACHE = {
...
def _load_decisions(limit: int = 20) -> list[str]:
    path = LIVE_STATE if _trading_mode() == "live" else PAPER_STATE
    try:
        data = json.loads(path.read_text(encoding="utf-8")) if path.exists() else {}
    except Exception:
        data = {}
    raw = data.get("last_decision_raw") or ""
...
@app.get("/api/journal")
async def api_journal(limit: int = 100, since_id: int | None = None):
    conn = _connect(_db_path())
    q = "SELECT id, ts, tick, instId, side, action, sz, px, leverage, pnl_usdt, rationale FROM trades"
    params: tuple[Any, ...] = ()
...
@app.get("/api/exits")
async def api_exits(limit: int = 20):
    conn = _connect(_db_path())
    rows = conn.execute(
        "SELECT id, ts, tick, instId, side, action, sz, px, leverage, pnl_usdt, rationale "
        "FROM trades WHERE pnl_usdt IS NOT NULL AND instId != '(no_trade)' "
...
async def api_overview():
    """Single payload for 500ms UI refresh, using cached account/positions."""
    pid = _read_pid(PID_FILE)
    alive = pid is not None and _is_process_alive(pid)
    mode = _trading_mode()
    if mode == "live":
...
async def api_start_trading():
    try:
        ...
        if not py.exists():
            py = Path("C:/Users/mknig/AppData/Local/Programs/Python/Python312/python.exe")
        if not py.exists():
        ...
        if sys.platform != "win32":
            cmd = ["/bin/bash", "-lc", "PYTHONPATH=" + str(BASE_DIR / "src") + " python -u -m emirald"]
...
async def api_launch():
    try:
        ...
        subprocess.Popen(
            [
                str(py),
                str(BASE_DIR / "scripts" / "launch_emirald_gui.py"),
                "--no-auto-start",
                "--working-dir",
                str(BASE_DIR),
            ],
            cwd=str(BASE_DIR),
            env={**os.environ, "EMIRALD_HOME": str(BASE_DIR)},
...
async def api_stop():
    try:
        ...
        if not py.exists():
            py = Path(sys.executable)
        subprocess.run(
            [str(py), str(BASE_DIR / "scripts" / "stop_emirald.py"), str(BASE_DIR), "--quiet"],
            check=False,
            capture_output=True,
        )
        lock = BASE_DIR / ".emirald_dashboard.lock"
        if lock.exists():
            try:
                lock.unlink()
            except Exception:
