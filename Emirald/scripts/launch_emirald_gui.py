"""Launch Emirald: visible control window + agent console + dashboard.
...
Dashboard serves http://127.0.0.1:8766/.
"""
...
PID_FILE = Path(".emirald.pid")
DASH_PID_FILE = Path(".emirald_dashboard.pid")
DASHBOARD_LOCK_FILE = Path(".emirald_dashboard.lock")
DASHBOARD_PORT = 8766
...
def _find_project_root(start: Path) -> Path:
    ...
    for parent in [root] + list(root.parents):
        if (parent / "config.yaml").exists() and (parent / "src" / "emirald").exists():
            return parent
    return root
...
def _find_emirald_agent_pids(root: Path) -> list[int]:
    ...
    for i, line in enumerate(out.splitlines()):
        ...
        if pid == os.getpid():
            continue
        if "emirald" not in cmd and root_s not in cmd:
            continue
        if "-m emirald" in cmd or "emirald.__main__" in cmd:
            found.append(pid)
    return found
...
def _stop_existing_agent(root: Path, *, prompt: bool = True) -> None:
    ...
    if stored and _pid_alive(stored):
        pids.append(stored)
    pids.extend(_find_emirald_agent_pids(root))
    ...
    if prompt:
        ...
        if not messagebox.askyesno("Emirald", f"Emirald agent already running (PID {joined}). Stop it and relaunch?"):
            sys.exit(0)
    for pid in pids:
        _terminate_pid(pid)
    _clear_pid(pid_path)
...
def _dashboard_up() -> bool:
    try:
        ...
        return "Emirald" in body and "KnightTrader" not in body
    except Exception:
        return False
...
def _start_dashboard(root: Path) -> subprocess.Popen | None:
    ...
    if _dashboard_up():
        return None
    # Free wrong listeners on Emirald port only.
    try:
        out = subprocess.check_output(
            [
                "powershell",
    ...
    proc = subprocess.Popen(
        ...
        cwd=str(root),
        env={**os.environ, "EMIRALD_HOME": str(root)},
...
def _start_bot(root: Path) -> subprocess.Popen:
    ...
    cmd = [str(exe), "-u", "-m", "emirald"]
    env = {**os.environ, "EMIRALD_HOME": str(root), "PYTHONUTF8": "1", "PYTHONIOENCODING": "utf-8"}
...
def _existing_agent_pid(root: Path) -> int | None:
    ...
    if stored and _pid_alive(stored):
        return stored
    found = _find_emirald_agent_pids(root)
    return found[0] if found else None


def _gui_lock_path(root: Path) -> Path:
    return root / ".emirald_gui.pid"
...
def _focus_existing_gui(root: Path) -> bool:
    """Best-effort: bring existing Emirald window to the foreground."""
...
def _parse_account_state(log_snippet: str) -> dict:
    ...
    return {
        ...
        "ready": "Emirald is awake" in log_snippet or "tick=" in log_snippet,
    }


class EmiraldGUI(tk.Tk):
    def __init__(self, root: Path, *, auto_start: bool = True):
        super().__init__()
        self.root = root
        self.log_path = root / "data" / "emirald.log"
    ...
    def _setup_ui(self):
        ...
        title = ttk.Label(self, text="Emirald Control Room", font=font.Font(size=18, weight="bold"))
        ...
        help_text.insert(
            ...
            "Use Stop Emirald desktop shortcut to kill agent + dashboard.\n"
            "Do not use port 8765 — that is another local stack.\n",
        )
    ...
    def _launch_dashboard_only(self, *, prompt_stop: bool = True):
        ...
        try:
            ...
            if _dashboard_up():
                ...
                messagebox.showerror("Emirald", "Dashboard failed to start on http://127.0.0.1:8766/")
        except Exception as exc:
            self.dash_var.set(f"Dashboard error: {exc}")
            messagebox.showerror("Emirald", f"Dashboard launch failed: {exc}")
    ...
    def _stop(self):
        ...
        if self.bot_process and self.bot_process.poll() is None:
            ...
            stopped = True
        for pid in _find_emirald_agent_pids(self.root):
            _terminate_pid(pid)
            stopped = True
        ...
        if not stopped:
            messagebox.showinfo("Emirald", "No active Emirald agent process.")
    ...
    def _on_close(self):
        ...
        # use Stop Emirald for that. Matches "launcher window" UX.
        self.destroy()


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Emirald Trader launcher")
    parser.add_argument("--working-dir", default=".", help="Emirald project root")
    ...
    app = EmiraldGUI(working, auto_start=not args.no_auto_start)
