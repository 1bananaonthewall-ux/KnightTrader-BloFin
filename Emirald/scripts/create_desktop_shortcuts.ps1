<#
.SYNOPSIS
Creates Emirald Dashboard, Emirald Trader (launcher), and Stop Emirald shortcuts
...
New-EmiraldShortcut -Name 'Emirald Dashboard' `
    -Arguments "-NoProfile -ExecutionPolicy Bypass -File `"$dashFile`"" `
    -Icon 'shell32.dll,21' `
    -WindowStyle 7

# Visible launcher window (python.exe, NOT pythonw / NOT Hidden)
$agentCmd = "& { Set-Location -LiteralPath '$root'; & '$python' '$scripts\launch_emirald_gui.py' '--working-dir' '$root' }"
New-EmiraldShortcut -Name 'Emirald Trader' `
    -Arguments "-NoProfile -ExecutionPolicy Bypass -Command `"$agentCmd`"" `
    -Icon 'shell32.dll,14' `
    -WindowStyle 1

$stopCmd = "& { Set-Location -LiteralPath '$root'; & '$python' '$scripts\stop_emirald.py' '$root' }"
New-EmiraldShortcut -Name 'Stop Emirald' `
