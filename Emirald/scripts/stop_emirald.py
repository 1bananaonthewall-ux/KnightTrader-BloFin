"""Stop Emirald agent, GUI launcher, and dashboard processes for this project."""
...
def _kill_matching(root: Path) -> list[int]:
    ...
    for pid, cmd in _process_commandlines():
        ...
        if "stop_emirald.py" in low:
            continue
        if root_s not in low and "emirald" not in low:
            if "apps.dashboard.main" not in low and "launch_emirald_gui.py" not in low:
                continue
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Stop Emirald agent + dashboard")
    ...
    for name in [PID_FILE.name, DASH_PID_FILE.name, ".emirald.lock", ".emirald_gui.pid"]:
        p = working / name
        if p.exists():
            try:
                p.unlink()
    ...
    msg = (
        f"Stopped Emirald (killed {len(unique)} process tree(s))."
        if unique
        else "No running Emirald agent/dashboard found. Cleared lock files."
    )
    if not args.quiet:
        try:
            ...
            root = tk.Tk()
            root.withdraw()
            messagebox.showinfo("Emirald", msg)
            root.destroy()
        except Exception:
            print(msg)
    else:
